# Restart Often Protocol (ROP)

Rooted in the "Commit early, commit often" philosophy, the Restart Often Protocol (ROP) is a method of working with AI Agents
that descreases costs and increases developer happiness.

The higher your ROP score, the leaner your architecture, and the further your compute budget stretches.

- [ROP.md](ROP.md) - Restart Often Protocol overview

# Opinionated High ROP Workflow (HROP)
- [HROP.md](HROP.md) - High ROP overview
- [AGENTS.md](AGENTS.md) - Agents Protocol
- [skills/](skills) - Agent skills for HROP
- [active-tasks/](active-tasks) - Active tasks and packets for this repo <https://github.com/attogram/rop>

## Welcome to HROP Task
- [active-tasks/welcome-to-rop/STATUS.md](active-tasks/welcome-to-rop/STATUS.md)
- [active-tasks/welcome-to-rop/TASK.md](active-tasks/welcome-to-rop/TASK.md)
- [active-tasks/welcome-to-rop/packets/](active-tasks/welcome-to-rop/packets)
